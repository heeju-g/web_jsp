...
function load(){
	...
	httpRequest = new XMLHttpRequest();    // XHR: 서버와 통신하기 위한 객체(javascript object) ->http를 통해 데이터송수신 지원
...
function callback(){
	...
	if(httpRequest.readyState == 4){
		...
		if(httpRequest.status == 200){
			var obj = JSON.parse(httpRequest.responseText); //response된 데이터(doc)
